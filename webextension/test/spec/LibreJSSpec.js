...
describe("LibreJS' components", () => {
  ...
  describe('The whitelist/blacklist manager', () => {
    ...
    let lm = new ListManager(
      new ListStore('_test.whitelist', Storage.CSV),
      new ListStore('_test.blacklist', Storage.CSV),
      new Set(),
    );
  ...
  describe('The HTML processor', () => {
    let processHtml = async (html, whitelisted = false) => LibreJS.editHtml(html, tab.url, tab.id, 0, whitelisted);

    let addScript = (html, script, before = '</head>') => html.replace(before, `<script>${script}</script>${before}`);
